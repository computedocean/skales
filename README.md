...
<p align="center">
 ...
 <p>
    <a href="https://github.com/skalesapp/skales/releases/latest"><img src="https://img.shields.io/badge/version-12.7.45-10b981?style=flat-square" alt="Version 12.7.45" /></a>
...
12.7.45 "SKLS" is the repair release for the one above it, built the day after from what the first devices reported. The local server the release was named for now ships for Apple Silicon as well, and answers with a local model id instead of a cloud one. The model list shows what the runtime actually holds, models on another drive are found instead of declared absent, and the Google chain tells one truth: the account connects without an error painted over the success, a tool that is named can be called, and credentials stay out of the transcript. A turn that ends with nothing now says what ended it, and a post generation that ran into a token limit stops calling it a timeout. Unsloth Desktop joins as a named provider.

12.7.4 "Skales Local" puts the inference server inside the app: no key, no account, no install. Chat, dictation, read-aloud and images can each be set to this machine first, cloud first, or off, and whoever answers is named before and after. Models are downloaded, imported or adopted from an existing Ollama or LM Studio folder, and every licence is in the app.

12.7.2 "Cockpit" is about seeing what is going on. Autopilot opens on an "In flight" list of everything running, an available update stays visible as a pill on every screen until you act on it, a tool call a model wrote as text can no longer render as the answer, WordPress grows from a posting box into running the whole site, the wait before a slow model's first token is filled with one short acknowledgment line, and mute now means mute - on every channel, at every priority, with held-back messages counted on the Notifications page instead of swallowed.

12.7.1 is a same-day hotfix on top of it, from two reports that arrived within minutes of the release. GigaChat now talks to the address that actually serves its current generation, and the Russian root certificate its endpoints are signed under ships with Skales instead of being something you have to go and find. Everything that names a model now names the one that is answering: the context readout, the status pill (which is a statement about whether a chat runs locally or in a cloud), the Code window and Lio. Switching the model with a command changes that chat instead of your settings, context windows come from the provider's own catalogue, and a lost connection is reported as a lost connection rather than as a crash.

12.7.0 is the release that brings the lightness back: on first start you choose what Skales should do for you, and the sidebar and settings show only that - Advanced still shows every screw. An interrupted answer says it was interrupted and offers retry, a local model may think in silence for minutes without being cut off, LM Studio has its own place in the provider list, Office files attach and share in, every chat carries an access mode down to read-only, Buddy greets new installs by default, and pairing a phone explains a failure in plain language instead of failing silently. GigaChat joins as a first-class provider. Skales now reads its own interface, so asking "where do I find X" gets an answer that matches the app you are actually looking at.
